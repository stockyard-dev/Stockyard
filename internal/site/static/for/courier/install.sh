...
echo ""
echo "  Stockyard for Couriers & Delivery Services"
...
BUNDLE_DIR="$HOME/stockyard-courier"
...
mkdir -p "$DATA"
echo ""
echo "  Starting Stockyard for Couriers & Delivery Services..."
echo ""
...
if command -v xdg-open &>/dev/null; then
  xdg-open "http://localhost:9809/ui" 2>/dev/null &
elif command -v open &>/dev/null; then
  open "http://localhost:9809/ui" 2>/dev/null &
fi
wait
STARTEOF
...
STOCKYARD FOR COURIERS & DELIVERY SERVICES
...
Trial:   https://stockyard.dev/pricing/?bundle=courier

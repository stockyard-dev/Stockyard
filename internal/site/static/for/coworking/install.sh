...
echo ""
echo "  ┌──────────────────────────────────────────┐"
echo "  │  Stockyard for Coworking Spaces         │"
echo "  │  8 tools · $7.99/mo · self-hosted        │"
echo "  │  https://stockyard.dev/for/coworking/    │"
echo "  └──────────────────────────────────────────┘"
echo ""
...
FAILED=0
  ...
  echo "  Installing Paddock..."
  if curl -fsSL "https://stockyard.dev/paddock/install.sh" 2>/dev/null | sh >/dev/null 2>&1; then
    echo "    ✓ Paddock"
  else
    echo "    ✗ Paddock (failed — try manually: curl stockyard.dev/paddock/install.sh | sh)"
    FAILED=$((FAILED + 1))
  fi
...
echo "  Free tier: 5 items per tool. Upgrade: stockyard.dev/pricing/?bundle=coworking"
echo ""

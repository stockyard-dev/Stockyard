#!/bin/sh
# Stockyard Permit installer
# Downloads the latest release from GitHub and installs to ~/.local/bin
#
# Usage:  curl -fsSL https://stockyard.dev/permit/install.sh | sh
#         INSTALL_DIR=/usr/local/bin curl -fsSL https://stockyard.dev/permit/install.sh | sh
set -e

...
REPO="stockyard-dev/stockyard-collection"
BINARY="stockyard-collection"
...
if [ -n "$TAG" ]; then
  ...
  if curl -fsSL "$URL" -o "${TMP}/${FILENAME}" 2>/dev/null; then
    ...
    echo "  Public collection: http://localhost:8840/book"
...
if command -v go &>/dev/null; then
  ...
  CGO_ENABLED=0 GOBIN="${INSTALL_DIR}" go install "github.com/${REPO}/cmd/collection@latest"

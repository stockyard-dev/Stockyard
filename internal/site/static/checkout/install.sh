...
REPO="stockyard-dev/stockyard-checkout"
BINARY="stockyard-checkout"
...
if [ -n "$TAG" ]; then
  ...
  if curl -fsSL "$URL" -o "${TMP}/${FILENAME}" 2>/dev/null; then
    ...
    echo "  Public checkout: http://localhost:8850/book"
...
if command -v go &>/dev/null; then
  ...
  CGO_ENABLED=0 GOBIN="${INSTALL_DIR}" go install "github.com/${REPO}/cmd/checkout@latest"

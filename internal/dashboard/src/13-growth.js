...
function GrowthView(){
  ...
  return html`<div class="page-head"><div class="page-eyebrow">Mission Control</div><h2>Growth Dashboard</h2><p class="page-sub">Internal traction metrics. Period: <strong>${pLabel}</strong></p></div>
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:24px">
      <div class="stat">
        <div class="stat-label">Installs (unique) ${srcBadge('auto')} ${windowTag('lifetime')}</div>
        <div class="stat-value accent">${fmtN(inst.unique)}</div>
        <div class="stat-sub">${fmtN(inst.last_7d?.unique||0)} in last 7d</div>
      </div>
      <div class="stat">
    ...
    <div style="display:grid;grid-template-columns:2fr 1fr;gap:16px;margin-bottom:24px">
      <div style="background:var(--bg2);border:1px solid var(--bg3);padding:16px">
        <div style="font-family:var(--font-mono);font-size:0.68rem;letter-spacing:2px;text-transform:uppercase;color:var(--rust);margin-bottom:8px">Installs Over Time ${srcBadge('auto')} ${windowTag(pLabel)}</div>

...
type TableForgeState struct {
	mu              sync.Mutex
	cfg             config.TableForgeConfig
	recentEvents    []TableForgeEvent
	tablesValidated atomic.Int64
	tablesRepaired  atomic.Int64
}
...
func (t *TableForgeState) Stats() map[string]any {
	t.mu.Lock()
	events := make([]TableForgeEvent, len(t.recentEvents))
	...
	return func(next proxy.Handler) proxy.Handler {
		return func(ctx context.Context, req *provider.Request) (*provider.Response, error) {
			...
			for _, c := range resp.Choices {
				...
				if len(lines) > 1 {
					t.tablesValidated.Add(1)
				}
			}
			t.mu.Lock()
			if len(t.recentEvents) >= 200 {

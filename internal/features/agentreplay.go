...
type AgentReplayState struct {
	mu                sync.Mutex
	cfg               config.AgentReplayConfig
	recentEvents      []AgentReplayEvent
	requestsProcessed atomic.Int64
}

func NewAgentReplay(cfg config.AgentReplayConfig) *AgentReplayState {
	return &AgentReplayState{cfg: cfg, recentEvents: make([]AgentReplayEvent, 0, 200)}
}
func (s *AgentReplayState) Stats() map[string]any {
	s.mu.Lock()
	events := make([]AgentReplayEvent, len(s.recentEvents))

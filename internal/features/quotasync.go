...
type QuotaSyncState struct {
	mu                sync.Mutex
	cfg               config.QuotaSyncConfig
	recentEvents      []QuotaSyncEvent
	requestsProcessed atomic.Int64
}

func NewQuotaSync(cfg config.QuotaSyncConfig) *QuotaSyncState {
	return &QuotaSyncState{cfg: cfg, recentEvents: make([]QuotaSyncEvent, 0, 200)}
}
func (s *QuotaSyncState) Stats() map[string]any {
	s.mu.Lock()
	events := make([]QuotaSyncEvent, len(s.recentEvents))

...
type ChaosLLMState struct {
	mu                sync.Mutex
	cfg               config.ChaosLLMConfig
	recentEvents      []ChaosLLMEvent
	requestsProcessed atomic.Int64
}

func NewChaosLLM(cfg config.ChaosLLMConfig) *ChaosLLMState {
	return &ChaosLLMState{cfg: cfg, recentEvents: make([]ChaosLLMEvent, 0, 200)}
}
func (s *ChaosLLMState) Stats() map[string]any {
	s.mu.Lock()
	events := make([]ChaosLLMEvent, len(s.recentEvents))

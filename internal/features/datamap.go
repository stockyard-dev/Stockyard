...
type DataMapState struct {
	mu                sync.Mutex
	cfg               config.DataMapConfig
	recentEvents      []DataMapEvent
	requestsProcessed atomic.Int64
}

func NewDataMap(cfg config.DataMapConfig) *DataMapState {
	return &DataMapState{cfg: cfg, recentEvents: make([]DataMapEvent, 0, 200)}
}
func (s *DataMapState) Stats() map[string]any {
	s.mu.Lock()
	events := make([]DataMapEvent, len(s.recentEvents))

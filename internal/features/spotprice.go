...
type SpotPriceState struct {
	mu                sync.Mutex
	cfg               config.SpotPriceConfig
	recentEvents      []SpotPriceEvent
	requestsProcessed atomic.Int64
}

func NewSpotPrice(cfg config.SpotPriceConfig) *SpotPriceState {
	return &SpotPriceState{cfg: cfg, recentEvents: make([]SpotPriceEvent, 0, 200)}
}
func (s *SpotPriceState) Stats() map[string]any {
	s.mu.Lock()
	events := make([]SpotPriceEvent, len(s.recentEvents))

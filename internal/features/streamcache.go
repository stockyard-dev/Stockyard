...
type StreamCacheState struct {
	...
	recentEvents  []StreamCacheEvent
	hits          atomic.Int64
	misses        atomic.Int64
	streamsStored atomic.Int64
}

func NewStreamCache(cfg config.StreamCacheConfig) *StreamCacheState {
	return &StreamCacheState{cfg: cfg, recentEvents: make([]StreamCacheEvent, 0, 200)}
}
func (s *StreamCacheState) Stats() map[string]any {
	s.mu.Lock()
	events := make([]StreamCacheEvent, len(s.recentEvents))

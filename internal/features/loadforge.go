...
type LoadForgeState struct {
	mu                sync.Mutex
	cfg               config.LoadForgeConfig
	recentEvents      []LoadForgeEvent
	requestsProcessed atomic.Int64
}

func NewLoadForge(cfg config.LoadForgeConfig) *LoadForgeState {
	return &LoadForgeState{cfg: cfg, recentEvents: make([]LoadForgeEvent, 0, 200)}
}
func (s *LoadForgeState) Stats() map[string]any {
	s.mu.Lock()
	events := make([]LoadForgeEvent, len(s.recentEvents))

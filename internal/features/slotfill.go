...
type SlotFillState struct {
	mu                sync.Mutex
	cfg               config.SlotFillConfig
	recentEvents      []SlotFillEvent
	slotsFilled       atomic.Int64
	sessionsCompleted atomic.Int64
}

func NewSlotFill(cfg config.SlotFillConfig) *SlotFillState {
	return &SlotFillState{cfg: cfg, recentEvents: make([]SlotFillEvent, 0, 200)}
}
func (s *SlotFillState) Stats() map[string]any {
	s.mu.Lock()
	events := make([]SlotFillEvent, len(s.recentEvents))

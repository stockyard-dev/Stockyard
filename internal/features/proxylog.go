...
type ProxyLogState struct {
	mu                sync.Mutex
	cfg               config.ProxyLogConfig
	recentEvents      []ProxyLogEvent
	requestsProcessed atomic.Int64
}

func NewProxyLog(cfg config.ProxyLogConfig) *ProxyLogState {
	return &ProxyLogState{cfg: cfg, recentEvents: make([]ProxyLogEvent, 0, 200)}
}
func (s *ProxyLogState) Stats() map[string]any {
	s.mu.Lock()
	events := make([]ProxyLogEvent, len(s.recentEvents))

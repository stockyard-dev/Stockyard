...
type ParamNormState struct {
	mu                sync.Mutex
	cfg               config.ParamNormConfig
	recentEvents      []ParamNormEvent
	requestsProcessed atomic.Int64
}

func NewParamNorm(cfg config.ParamNormConfig) *ParamNormState {
	return &ParamNormState{cfg: cfg, recentEvents: make([]ParamNormEvent, 0, 200)}
}
func (s *ParamNormState) Stats() map[string]any {
	s.mu.Lock()
	events := make([]ParamNormEvent, len(s.recentEvents))

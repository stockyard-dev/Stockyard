...
type SummarizeGateEvent struct {
...
type SummarizeGateState struct {
	mu                sync.Mutex
	cfg               config.SummarizeGateConfig
	recentEvents      []SummarizeGateEvent
	requestsProcessed atomic.Int64
	tokensSaved       atomic.Int64
}

func NewSummarizeGate(cfg config.SummarizeGateConfig) *SummarizeGateState {
	return &SummarizeGateState{cfg: cfg, recentEvents: make([]SummarizeGateEvent, 0, 200)}
}
func (s *SummarizeGateState) Stats() map[string]any {
	s.mu.Lock()
	events := make([]SummarizeGateEvent, len(s.recentEvents))

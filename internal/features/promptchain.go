...
type PromptChainState struct {
	mu              sync.Mutex
	cfg             config.PromptChainConfig
	recentEvents    []PromptChainEvent
...
func (p *PromptChainState) Stats() map[string]any {
	p.mu.Lock()
	events := make([]PromptChainEvent, len(p.recentEvents))
			...
			for i, m := range req.Messages {
				if m.Role == "system" {
					...
					for name, block := range p.blocks {
						content = strings.ReplaceAll(content, "{{"+name+"}}", block)
					}
					p.mu.Unlock()
					if content != m.Content {
						req.Messages[i].Content = content
						p.compositionsRun.Add(1)
					}
				}
			}
			return next(ctx, req)

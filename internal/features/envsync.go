...
type EnvSyncState struct {
	mu                sync.Mutex
	cfg               config.EnvSyncConfig
	recentEvents      []EnvSyncEvent
	requestsProcessed atomic.Int64
}

func NewEnvSync(cfg config.EnvSyncConfig) *EnvSyncState {
	return &EnvSyncState{cfg: cfg, recentEvents: make([]EnvSyncEvent, 0, 200)}
}
func (s *EnvSyncState) Stats() map[string]any {
	s.mu.Lock()
	events := make([]EnvSyncEvent, len(s.recentEvents))

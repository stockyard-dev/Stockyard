...
type CostMapState struct {
	mu                sync.Mutex
	cfg               config.CostMapConfig
	recentEvents      []CostMapEvent
	requestsProcessed atomic.Int64
}

func NewCostMap(cfg config.CostMapConfig) *CostMapState {
	return &CostMapState{cfg: cfg, recentEvents: make([]CostMapEvent, 0, 200)}
}
func (s *CostMapState) Stats() map[string]any {
	s.mu.Lock()
	events := make([]CostMapEvent, len(s.recentEvents))

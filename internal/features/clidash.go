...
type CliDashState struct {
	mu                sync.Mutex
	cfg               config.CliDashConfig
	recentEvents      []CliDashEvent
	requestsProcessed atomic.Int64
}

func NewCliDash(cfg config.CliDashConfig) *CliDashState {
	return &CliDashState{cfg: cfg, recentEvents: make([]CliDashEvent, 0, 200)}
}
func (s *CliDashState) Stats() map[string]any {
	s.mu.Lock()
	events := make([]CliDashEvent, len(s.recentEvents))

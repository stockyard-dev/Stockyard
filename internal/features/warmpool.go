...
type WarmPoolState struct {
	...
	recentEvents      []WarmPoolEvent
	requestsProcessed atomic.Int64
}

func NewWarmPool(cfg config.WarmPoolConfig) *WarmPoolState {
	return &WarmPoolState{cfg: cfg, recentEvents: make([]WarmPoolEvent, 0, 200)}
}
func (s *WarmPoolState) Stats() map[string]any {
	s.mu.Lock()
	events := make([]WarmPoolEvent, len(s.recentEvents))

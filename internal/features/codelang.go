...
type CodeLangState struct {
	mu                sync.Mutex
	cfg               config.CodeLangConfig
	recentEvents      []CodeLangEvent
	requestsProcessed atomic.Int64
}

func NewCodeLang(cfg config.CodeLangConfig) *CodeLangState {
	return &CodeLangState{cfg: cfg, recentEvents: make([]CodeLangEvent, 0, 200)}
}
func (s *CodeLangState) Stats() map[string]any {
	s.mu.Lock()
	events := make([]CodeLangEvent, len(s.recentEvents))

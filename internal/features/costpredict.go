...
type CostPredictState struct {
	mu           sync.Mutex
	cfg          config.CostPredictConfig
	recentEvents []CostPredictEvent
	predictions  atomic.Int64
	blocked      atomic.Int64
}

func NewCostPredict(cfg config.CostPredictConfig) *CostPredictState {
	return &CostPredictState{cfg: cfg, recentEvents: make([]CostPredictEvent, 0, 200)}
}
func (c *CostPredictState) Stats() map[string]any {
	c.mu.Lock()
	events := make([]CostPredictEvent, len(c.recentEvents))

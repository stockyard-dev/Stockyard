...
type PersonaSwitchState struct {
	mu              sync.Mutex
	cfg             config.PersonaSwitchConfig
	recentEvents    []PersonaSwitchEvent
...
func (ps *PersonaSwitchState) Stats() map[string]any {
	ps.mu.Lock()
	events := make([]PersonaSwitchEvent, len(ps.recentEvents))
			...
			if req.Project != "" {
				...
				if p, ok := ps.personas[req.Project]; ok {
					...
					for i, m := range req.Messages {
						if m.Role == "system" {
							req.Messages[i].Content = p.SystemPrompt + "\n" + m.Content
							found = true
							break
						}
					}

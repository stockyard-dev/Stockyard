...
type PromptFuzzState struct {
	mu           sync.Mutex
	cfg          config.PromptFuzzConfig
	recentEvents []PromptFuzzEvent
	testsRun     atomic.Int64
	testsFailed  atomic.Int64
}

func NewPromptFuzz(cfg config.PromptFuzzConfig) *PromptFuzzState {
	return &PromptFuzzState{cfg: cfg, recentEvents: make([]PromptFuzzEvent, 0, 200)}
}
func (p *PromptFuzzState) Stats() map[string]any {
	p.mu.Lock()
	events := make([]PromptFuzzEvent, len(p.recentEvents))

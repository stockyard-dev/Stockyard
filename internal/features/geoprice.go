...
type GeoPriceState struct {
	mu                sync.Mutex
	cfg               config.GeoPriceConfig
	recentEvents      []GeoPriceEvent
	requestsProcessed atomic.Int64
}

func NewGeoPrice(cfg config.GeoPriceConfig) *GeoPriceState {
	return &GeoPriceState{cfg: cfg, recentEvents: make([]GeoPriceEvent, 0, 200)}
}
func (s *GeoPriceState) Stats() map[string]any {
	s.mu.Lock()
	events := make([]GeoPriceEvent, len(s.recentEvents))

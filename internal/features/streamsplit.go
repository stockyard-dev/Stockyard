...
type StreamSplitState struct {
	mu                sync.Mutex
	cfg               config.StreamSplitConfig
	recentEvents      []StreamSplitEvent
	requestsProcessed atomic.Int64
}

func NewStreamSplit(cfg config.StreamSplitConfig) *StreamSplitState {
	return &StreamSplitState{cfg: cfg, recentEvents: make([]StreamSplitEvent, 0, 200)}
}
func (s *StreamSplitState) Stats() map[string]any {
	s.mu.Lock()
	events := make([]StreamSplitEvent, len(s.recentEvents))

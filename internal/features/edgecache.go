...
type EdgeCacheState struct {
	...
	recentEvents      []EdgeCacheEvent
	requestsProcessed atomic.Int64
}

func NewEdgeCache(cfg config.EdgeCacheConfig) *EdgeCacheState {
	return &EdgeCacheState{cfg: cfg, recentEvents: make([]EdgeCacheEvent, 0, 200)}
}
func (s *EdgeCacheState) Stats() map[string]any {
	s.mu.Lock()
	events := make([]EdgeCacheEvent, len(s.recentEvents))

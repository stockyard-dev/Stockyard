...
type ConvoForkState struct {
	mu                sync.Mutex
	cfg               config.ConvoForkConfig
	recentEvents      []ConvoForkEvent
	forksCreated      atomic.Int64
	requestsProcessed atomic.Int64
}

func NewConvoFork(cfg config.ConvoForkConfig) *ConvoForkState {
	return &ConvoForkState{cfg: cfg, recentEvents: make([]ConvoForkEvent, 0, 200)}
}
func (c *ConvoForkState) Stats() map[string]any {
	c.mu.Lock()
	events := make([]ConvoForkEvent, len(c.recentEvents))

...
func RegisterAll(h *Hub) {
	products := []Product{
		...
		// ── New Products ──
		{ID: "relic", Name: "Relic", Description: "Content provenance chain", Category: "quality",
			Capabilities: []string{"request.completed"}, Emits: []string{"provenance.certified"},
			Status: StatusEmbedded},
		{ID: "breed", Name: "Breed", Description: "Genetic prompt evolution", Category: "lifecycle",
			Capabilities: []string{"quality.scored", "request.completed"}, Emits: []string{"prompt.evolved"},
			Status: StatusEmbedded},
		{ID: "fossilrec", Name: "Fossil Record", Description: "Temporal model archaeology", Category: "quality",
			Capabilities: []string{"request.completed"}, Emits: []string{"model.drift.detected", "model.fingerprint.updated"},
			Status: StatusEmbedded},
		{ID: "phantom", Name: "Phantom", Description: "Persistent AI canaries", Category: "quality",
			Capabilities: []string{"deploy.completed", "error.detected"}, Emits: []string{"phantom.anomaly", "phantom.report"},
			Status: StatusEmbedded},
		{ID: "feral", Name: "Feral", Description: "Adversarial AI hunter", Category: "quality",
			Capabilities: []string{"guardrail.blocked", "patch.applied"}, Emits: []string{"attack.discovered", "vulnerability.found"},
			Status: StatusEmbedded},
		{ID: "tidepool", Name: "Tide Pool", Description: "Emergent behavior detection", Category: "insight",
			Capabilities: []string{"request.completed", "error.detected", "quality.scored"}, Emits: []string{"feedback.loop.detected", "emergent.behavior.found"},
			Status: StatusEmbedded},
		{ID: "crucible", Name: "Crucible", Description: "System-level confidence scoring", Category: "quality",
			Capabilities: []string{"request.completed", "quality.scored", "confidence.low"}, Emits: []string{"system.confidence.scored", "pipeline.degraded"},
			Status: StatusEmbedded},
		{ID: "cortex", Name: "Cortex", Description: "Shared AI memory substrate", Category: "insight",
			Capabilities: []string{"request.completed", "query.answered", "decision.made"}, Emits: []string{"memory.updated", "knowledge.propagated"},
			Status: StatusEmbedded},
		{ID: "mycelium", Name: "Mycelium", Description: "Cross-instance intelligence", Category: "infra",
			Capabilities: []string{"model.drift.detected", "attack.discovered", "error.detected"}, Emits: []string{"network.insight"},
			Status: StatusEmbedded},
		{ID: "spore", Name: "Spore", Description: "Self-replicating patterns", Category: "infra",
			Capabilities: []string{"error.diagnosed", "patch.applied"}, Emits: []string{"pattern.captured", "pattern.activated"},
			Status: StatusEmbedded},
		{ID: "molt", Name: "Molt", Description: "Automatic architecture shedding", Category: "infra",
			Capabilities: []string{"request.completed", "pressure.high"}, Emits: []string{"component.shed", "waste.detected"},
			Status: StatusEmbedded},
	}

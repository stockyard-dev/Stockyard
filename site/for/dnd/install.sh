...
mkdir -p "$DATA"
echo ""
echo "  Starting Stockyard for D&D & TTRPG Groups..."
echo ""
...
if command -v xdg-open &>/dev/null; then
  xdg-open "http://localhost:9370/ui" 2>/dev/null &
elif command -v open &>/dev/null; then
  open "http://localhost:9370/ui" 2>/dev/null &
fi
wait
STARTEOF
...
STOCKYARD FOR D&D & TTRPG GROUPS
...
Trial:   https://stockyard.dev/pricing/?bundle=dnd

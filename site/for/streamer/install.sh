...
BUNDLE_DIR="$HOME/stockyard-streamer"
...
mkdir -p "$DATA"
echo ""
echo "  Starting Stockyard for Twitch & YouTube Streamers..."
echo ""
...
PORT=9880 "$DIR/tools/stockyard-prospector" -port 9880 -data "$DATA" >/dev/null 2>&1 &
PORT=8560 "$DIR/tools/stockyard-dispatch" -port 8560 -data "$DATA" >/dev/null 2>&1 &
sleep 1
echo ""
...
STOCKYARD FOR TWITCH & YOUTUBE STREAMERS
...
Trial:   https://stockyard.dev/pricing/?bundle=streamer

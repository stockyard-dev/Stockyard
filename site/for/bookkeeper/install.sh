...
echo "  │  https://stockyard.dev/for/bookkeeper/   │"
echo "  └──────────────────────────────────────────┘"
echo ""
...
FAILED=0
  ...
  if curl -fsSL "https://stockyard.dev/ledger/install.sh" 2>/dev/null | sh >/dev/null 2>&1; then
    echo "    ✓ Ledger"
  else
    echo "    ✗ Ledger (failed — try manually: curl stockyard.dev/ledger/install.sh | sh)"
    FAILED=$((FAILED + 1))
  fi
...
echo "  Free tier: 5 items per tool. Upgrade: stockyard.dev/pricing/?bundle=bookkeeper"
echo ""

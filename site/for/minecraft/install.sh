...
BUNDLE_DIR="$HOME/stockyard-minecraft"
...
echo "  Downloading Corral..."
URL="https://github.com/stockyard-dev/stockyard-corral/releases/latest/download/stockyard-corral_${OS}_${ARCH}.tar.gz"
if curl -fsSL "$URL" -o "$TMP/archive.tar.gz" 2>/dev/null; then
  tar -xzf "$TMP/archive.tar.gz" -C "$TMP" 2>/dev/null
  mv "$TMP/stockyard-corral_${OS}_${ARCH}" "$BUNDLE_DIR/tools/stockyard-corral" 2>/dev/null || \
  mv "$TMP/stockyard-corral" "$BUNDLE_DIR/tools/stockyard-corral" 2>/dev/null || true
  chmod +x "$BUNDLE_DIR/tools/stockyard-corral" 2>/dev/null
  rm -f "$TMP/archive.tar.gz"
  echo "    ✓ Corral"
else
  echo "    ✗ Corral (failed)"
...
echo "  ✓ Corral                    http://localhost:8760/ui"
...
pkill -f "stockyard-corral" 2>/dev/null && echo "  ✓ Stopped Corral" || true
...
STOCKYARD FOR MINECRAFT SERVER ADMINS
...
Trial:   https://stockyard.dev/pricing/?bundle=minecraft

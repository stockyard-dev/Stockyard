...
echo ""
echo "  Stockyard for Leatherworkers"
...
BUNDLE_DIR="$HOME/stockyard-leather-worker"
...
mkdir -p "$DATA"
echo ""
echo "  Starting Stockyard for Leatherworkers..."
echo ""
...
if command -v xdg-open &>/dev/null; then
  xdg-open "http://localhost:9808/ui" 2>/dev/null &
elif command -v open &>/dev/null; then
  open "http://localhost:9808/ui" 2>/dev/null &
fi
wait
STARTEOF
...
Trial:   https://stockyard.dev/pricing/?bundle=leather-worker

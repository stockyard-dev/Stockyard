...
if [ -z "$TAG" ]; then
  ...
  if command -v go &>/dev/null; then
    CGO_ENABLED=0 GOBIN="${INSTALL_DIR}" go install "github.com/${REPO}/cmd/trough@latest"
...
echo ""
echo "  Stockyard Trough installed to ${INSTALL_DIR}/${BINARY}"
echo "  Quick start:  DATA_DIR=./data stockyard-trough"

// Stockyard Seance -- Talk to your production system like it's a person.
//
// Connect data sources (health endpoints, metrics, logs, databases, Redis,
// Prometheus). Ask questions in natural language. Get answers synthesized
// from live system state.
//
// Usage:
//
...
func cmdAsk(question string) {
	...
	streamer := stream.New(llm, model)
	tokenCh, err := streamer.AskStream(context.Background(), question, state, nil)
	if err != nil {
		fmt.Printf("  Error: %v\n", err)
		os.Exit(1)
	}
	...
	fmt.Print("  ")
	var metadata *stream.TokenMetadata
	for token := range tokenCh {
		...
		if token.Done && token.Metadata != nil {
	...
	fmt.Println()
	if metadata != nil {
		fmt.Printf("\n  Sources: %s\n", strings.Join(metadata.SourcesUsed, ", "))
		fmt.Printf("  Confidence: %s\n", metadata.Confidence)
		fmt.Printf("  Data: %d sources queried in %dms, answer in %dms\n",
			metadata.DataSources, metadata.CollectMs, metadata.LatencyMs)
	}
	fmt.Println()
}

func cmdStatus() {
	coll, engine, _, _ := setup()
...
func cmdChat() {
	...
	for {
		...
		tokenCh, err := streamer.AskStream(context.Background(), question, state, history)
		if err != nil {
			fmt.Printf("  Error: %v\n\n", err)
			continue
		}
		...
		if metadata != nil {
			fmt.Printf("\n  [%s confidence, %d sources, %dms]\n\n",
				metadata.Confidence, metadata.DataSources, metadata.LatencyMs)
		} else {
			fmt.Println()
			fmt.Println()
		}
		...
		if resp := fullResponse.String(); resp != "" {
			history = append(history, synth.QAPair{Question: question, Answer: resp})
			if len(history) > 10 {
...
func cmdServe(args []string) {
	...
	fmt.Printf("    Stream:    http://localhost:%d/api/ask/stream?q=...\n\n", *port)
...
func setup() (*collector.Collector, *synth.Engine, provider.Provider, string) {
...
func autoDiscoverSources(coll *collector.Collector) {
	...
	// Prometheus
	if url := os.Getenv("SEANCE_PROMETHEUS_URL"); url != "" {
		coll.Add(connector.NewPrometheusSource("prometheus", connector.PrometheusSourceConfig{
			Endpoint: url,
		}))
	}

	// Redis
	if addr := os.Getenv("SEANCE_REDIS_ADDR"); addr != "" {
		coll.Add(connector.NewRedisSource("redis", connector.RedisSourceConfig{
			Addr:     addr,
			Password: os.Getenv("SEANCE_REDIS_PASSWORD"),
		}))
	}

	// SQL Database
	if dsn := os.Getenv("SEANCE_SQL_DSN"); dsn != "" {
		driver := os.Getenv("SEANCE_SQL_DRIVER")
		if driver == "" {
			driver = "postgres"
		}
		coll.Add(connector.NewSQLSource("database", connector.SQLSourceConfig{
			DSN:    dsn,
			Driver: driver,
		}))
	}
...
func loadSourcesFromConfig(coll *collector.Collector, path string) {
	...
	type sourceConfig struct {
		...
		Type      string            `json:"type"` // http, health, log, env, static, sql, redis, prometheus
	...
	for _, cfg := range configs {
		...
		case "sql":
			coll.Add(connector.NewSQLSource(cfg.Name, connector.SQLSourceConfig{
				DSN:     cfg.DSN,
				Driver:  cfg.Driver,
				Queries: cfg.Queries,
			}))
		case "redis":
			coll.Add(connector.NewRedisSource(cfg.Name, connector.RedisSourceConfig{
				Addr:     cfg.Addr,
		...
		case "prometheus":
			...
			coll.Add(connector.NewPrometheusSource(cfg.Name, connector.PrometheusSourceConfig{
				Endpoint: endpoint,
				Queries:  cfg.Queries,
				Headers:  cfg.Headers,
			}))
...
func printUsage() {
  ...
  Usage:
    seance ask <question>            Ask your system a question (streams output)
    seance status                    Get a system status summary
    seance chat                      Interactive conversation mode (streaming)
    seance serve [flags]             Start web UI with SSE streaming
  ...
  Environment:
    ...
    SEANCE_PROMETHEUS_URL   Prometheus endpoint for metrics queries
    SEANCE_REDIS_ADDR       Redis address (host:port)
    SEANCE_REDIS_PASSWORD   Redis password
    SEANCE_SQL_DSN          SQL database connection string
    SEANCE_SQL_DRIVER       SQL driver (postgres, mysql, sqlite)
  ...
  Config file (~/.seance/sources.json):
    [
      ...
      {"name": "services", "type": "health", "endpoints": {"api": "...", "db": "..."}},
      {"name": "db", "type": "sql", "driver": "postgres", "dsn": "postgres://...", "queries": {"active_conns": "SELECT count(*) FROM pg_stat_activity"}},
      {"name": "cache", "type": "redis", "addr": "localhost:6379", "patterns": ["app:*"]},
      {"name": "metrics", "type": "prometheus", "endpoint": "http://prometheus:9090", "queries": {"error_rate": "sum(rate(http_errors_total[5m]))"}}
    ]
  ...
  Examples:
    ...
    seance ask "what is the current error rate?"
    seance status
    seance chat`)
}

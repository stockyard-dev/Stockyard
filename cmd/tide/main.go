...
import (
	...
	"github.com/stockyard-dev/stockyard/internal/tide/config"
...
func cmdDemo() {
	...
	for _, p := range pressures {
		...
		stats := engine.Stats()
		fmt.Printf("  Pressure: %.0f%%  Active: %d/%d  Hibernating: %d  Waking: %d", p*100, stats.Active, stats.Total, stats.Hibernating, stats.Waking)
		if len(events) > 0 {
			fmt.Printf("  ->")
...
func cmdServe(args []string) {
	...
	configPath := fs.String("config", "", "Path to tide.yaml config file")
	fs.Parse(args)
	...
	if *configPath != "" {
		...
		if err != nil {
			log.Fatalf("tide: load config: %v", err)
		}
		if cfg.Threshold > 0 {
	...
	if cfg != nil && len(cfg.Features) > 0 {
		...
		for _, fc := range cfg.Features {
			engine.Register(metabolic.Feature{
				...
				CPUWeight: fc.CPUWeight,
				MemWeight: fc.MemWeight,
...
func printUsage() {
  ...
  Serve flags:
    ...
    --config <path>        Path to tide.yaml config file`)
}
